...
components_to_ordinal :: proc(year, month, day: int) -> (ordinal: Ordinal, err: Error) {
...
components_to_date :: proc(year, month, day: int) -> (date: Date, err: Error) {
...
year_end :: proc(year: int) -> (year_end: Date, err: Error) {
	year_end = {year, 12, 31}
...
unsafe_ordinal_to_year :: proc(ordinal: Ordinal) -> (year: int, day_ordinal: int) {
	// Days after epoch
	d0   := ordinal - EPOCH
